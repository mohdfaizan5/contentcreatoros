...
import { CheckCircleIcon, CircleIcon, ClockIcon, LayoutIcon, TagChevronIcon } from '@phosphor-icons/react/dist/ssr'
...
const STATUS_META: Record<string, { icon: any; colorClass: string }> = {
  done: { icon: CheckCircleIcon, colorClass: 'text-emerald-500' },
  completed: { icon: CheckCircleIcon, colorClass: 'text-emerald-500' },
  'in-progress': { icon: ClockIcon, colorClass: 'text-amber-500' },
  in_progress: { icon: ClockIcon, colorClass: 'text-amber-500' },
  todo: { icon: CircleIcon, colorClass: 'text-zinc-400' },
}

function statusMeta(status: string) {
  return STATUS_META[status.toLowerCase()] ?? { icon: CircleIcon, colorClass: 'text-zinc-400' }
}
    ...
    <div className="border-b last:border-b-0">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <LayoutIcon className="size-4 shrink-0 text-primary" />
