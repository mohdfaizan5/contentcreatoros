...
import { deleteTemplate, updateTemplate } from '@/features/templates/actions/templates';
...
import type { Template, PlatformType } from '@/shared/types/database';
...
function TemplateMasonryCard({ template, isPinned = false }: TemplateMasonryCardProps) {
    ...
    return (
        <Link
            href={buildAppPath(LEGACY_APP_ROOT, `/templates/${template.id}`)}
    ...
    if (templates.length === 0) {
        return (
            <div className="text-center py-16 rounded-2xl border border-dashed bg-muted/20">
                ...
                <Link href={buildAppPath(LEGACY_APP_ROOT, '/templates/create')}>
