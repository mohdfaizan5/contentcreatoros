...
} from '@/features/(legacy)/templates/generated-tweets';
...
export function BrandTweetStudio({
  ...
  const handlePostNowWithApi = (tweetId: string) => {
    if (!selectedXAccountId) {
      setPublishNowError('Choose a founder or company X account before publishing.');
      return;
    }
    ...
    startPostNowTransition(async () => {
      try {
        const publishedTweet = await publishGeneratedTweetNow(tweetId, selectedXAccountId);
          ...
          {canAutoSchedule ? (
            <div className="flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700">
              ...
              {selectedXAccount
                ...
                : 'Choose founder or company before publishing'}
            </div>
          ) : (
