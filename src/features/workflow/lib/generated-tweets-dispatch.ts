...
import { publishTweetWithStoredConnection } from '@/features/x/lib/x';
import type { PostMediaAttachment, ScheduledDispatchRunStatus } from '@/shared/types/database';
...
export async function dispatchScheduledTweets(
  ...
  try {
    ...
    for (const row of (dueTweets ?? []) as ScheduledTweetRow[]) {
      ...
      if (!accountId) {
        const message =
          'Missing x_account_id for this scheduled tweet. Re-select a founder or company account and reschedule this post.';
