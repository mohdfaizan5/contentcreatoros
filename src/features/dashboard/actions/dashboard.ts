...
import { getAuthenticatedUserTweets, getAuthenticatedXUser } from '@/features/x/lib/x';
import type { GeneratedTweetStatus } from '@/shared/types/database';
...
export async function getDashboardSnapshot(): Promise<DashboardSnapshot> {
  ...
  const [onboardingCountResponse, generatedTweetsResponse, xAccounts] =
    await Promise.all([
      supabase
        .from('onboarding_answers')
  ...
  const preferredXAccount =
    xAccounts.find((account) => account.account_role === 'company') ??
    xAccounts.find((account) => account.account_role === 'founder') ??
    null;
  ...
  if (preferredXAccount?.id) {
    try {
      const accessToken = await ensureStoredXAccessToken(preferredXAccount.id);
...
export async function scheduleSevenDayContentPlan(params: {
  ...
  if (!params.xAccountId) {
    throw new Error('Choose a founder or company X account before scheduling this plan.');
  }

  const { data: xAccount } = await supabase
    .from('x_accounts')
    .select('id, account_role')
    .eq('id', params.xAccountId)
    .eq('user_id', user.id)
    .maybeSingle();

  if (!xAccount?.id || !xAccount.account_role) {
    throw new Error(
      'Reconnect the selected founder or company X account before scheduling this plan.',
    );
  }
