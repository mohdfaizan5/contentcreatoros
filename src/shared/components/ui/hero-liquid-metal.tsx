...
export function HeroLiquidMetalVisual({
  ...
  return (
    <div
      className={cn(
        "relative hidden h-87.5 lg:block lg:h-100 xl:h-125",
        className
      )}
...
export function HeroLiquidMetalMobileVisual({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-x-0 -bottom-24 -z-10 h-90 overflow-hidden lg:hidden",
        className
      )}
      data-slot="hero-liquid-metal-mobile"
      {...props}
    >
      <div className="absolute inset-x-0 top-0 z-10 h-56 bg-linear-to-b from-background via-background/95 to-transparent" />
      <MemoizedLiquidMetal
...
export function HeroLiquidMetal({
  ...
  // Keep these props consumed for backwards compatibility while this composed
  // component now renders only the shader layers.
  const unusedLayoutProps = [
    containerClassName,
    contentClassName,
    headingWrapClassName,
    headingClassName,
  ...
  void unusedLayoutProps

  return (
    <HeroLiquidMetalRoot {...props}>
      <HeroLiquidMetalShaderLayers
        mobileVisualClassName={mobileVisualClassName}
...
function HeroLiquidMetalShaderLayers({
  visualClassName,
  mobileVisualClassName,
}: {
  visualClassName?: string
  ...
  return (
    <div
      className="relative h-90 w-full lg:h-125"
      data-slot="hero-liquid-metal-shader-layers"
    >
      <div
        className={cn(
          "pointer-events-none absolute inset-0 hidden overflow-hidden lg:flex lg:items-center lg:justify-center",
          visualClassName
        )}
        data-slot="hero-liquid-metal-desktop-only"
      >
        <MemoizedLiquidMetal
          {...context.mergedDesktopShaderProps}
          image={
            context.mergedDesktopShaderProps.image ??
            (defaultDesktopShaderProps.image as string)
          }
        />
      </div>
      ...
      <div
        ...
        data-slot="hero-liquid-metal-mobile-only"
      >
        <div className="absolute inset-x-0 top-0 z-10 h-56 bg-linear-to-b from-background via-background/95 to-transparent" />
        <MemoizedLiquidMetal
          {...context.mergedMobileShaderProps}
          image={
            context.mergedMobileShaderProps.image ??
            (defaultMobileShaderProps.image as string)
          }
        />
      </div>
