...
import { useComposedRefs } from "@/features/inspiration/lib/compose-refs";
    ...
    return {
      ...
      setOpen: (value: boolean) => {
        ...
        if (propsRef.current.onOpenChange) {
          propsRef.current.onOpenChange(value, { type: 'OpenChange' } as never);
        }
...
function ColorPickerContent(
  ...
  if (context.inline) {
    return (
      <div
        ...
        {...(popoverContentProps as React.HTMLAttributes<HTMLDivElement>)}
        className={cn("flex w-[340px] flex-col gap-4 p-4", className)}
        style={popoverContentProps.style as React.CSSProperties}
      >
        {children}
      </div>
    );
  ...
  return (
    <PopoverContent
      data-slot="color-picker-content"
      {...(popoverContentProps as React.ComponentProps<typeof PopoverContent>)}
      className={cn("flex w-[340px] flex-col gap-4 p-4", className)}
    >
      {children}
    </PopoverContent>
