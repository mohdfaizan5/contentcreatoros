...
import { ArrowLeft, SpinnerGap, TwitterLogo, YoutubeLogo, LinkedinLogo, Article } from '@phosphor-icons/react';
import Link from 'next/link';
import { Button } from '@/shared/components/ui/button';
import { AutoResizeTextarea } from '@/shared/components/ui/auto-resize-textarea';
import { createSeries } from '@/features/(legacy)/series/actions/series';
...
const platforms = [
    ...
    { value: '', icon: Article, label: 'Multi-platform', color: 'hover:bg-gray-500/10 hover:text-gray-500 data-[active=true]:bg-gray-500/10 data-[active=true]:text-gray-500 data-[active=true]:border-border/30' },
];
...
export default function CreateSeriesPage() {
    ...
    const handleSubmit = () => {
        ...
        startTransition(async () => {
            try {
                ...
                router.push(buildAppPath(LEGACY_APP_ROOT, '/series'));
            } catch (error) {
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6 animate-fade-in-up">
            {/* Back navigation */}
            <Link
                href={buildAppPath(LEGACY_APP_ROOT, '/series')}
